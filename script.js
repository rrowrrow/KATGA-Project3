...
function createKeyboard() {
  ...
  layouts.forEach((layout) => {
    ...
    layout.forEach((keyValue) => {
      ...
      btn.setAttribute("aria-label", keyValue === "⌫" ? "Hapus satu huruf" : keyValue);
...
function showModal({ eyebrow, title, body, actions = [], skipQueue = false }) {
  ...
  setTimeout(() => {
    const firstButton = els.modalActions.querySelector("button") || els.modalCloseBtn;
